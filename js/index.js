...
const ELEMENT_MODAL_DATA = [
    ...
    },
    {
        ...
        "spectral_img": "https://en.wikipedia.org/wiki/File:Beryllium_Spectra.jpg",
        "summary": "Beryllium is a silvery-white metal. It is relatively soft and has a low density. Beryllium is used in alloys with copper or nickel to make gyroscopes, springs, electrical contacts, spot-welding electrodes and non-sparking tools. Mixing beryllium with these metals increases their electrical and thermal conductivity.",
        "symbol": "Be",
        "xpos": 1,
        "ypos": 1,
        "shells": [
...
Array.from(elementList).forEach(elementItem => {
    elementItem.addEventListener('click', () => {
        if (isLanthanoidOrActinoid(elementItem)) {
            ...
            const elementName = elementClicked.getAttribute('data-element-name');
...
function createModalContent(elementName) {
    const elementSelected = ELEMENT_MODAL_DATA.find(x => x.name === elementName);
    const element = transformContent(elementSelected);
    Object.keys(element).forEach(key => modalProperties.innerHTML += `<p class='element-property element-${key}'><span class="property-key-label">${key}</span>: ${element[key]}</p>`);
}

function transformContent(data) {
    return {
        "Name": data.name,
        "Automic mass": data.atomic_mass,
        "Boil": data.boil,
        "Category": data.category,
        "Color": data.color || 'None',
        "Discovered by": data.discovered_by,
        "Melt": data.melt,
        "Molar heat": data.molar_heat,
        "Named by": data.named_by || 'Unknown',
        "Number": data.number,
        "Period": data.period,
        "Phase": data.phase,
        "Symbol": data.symbol,
        "Electron affinity": data.electron_affinity,
        "Electronegativity pauling": data.electronegativity_pauling,
        "Electron configuration": data.electron_configuration,
        "Electron configuration semantic": data.electron_configuration_semantic,
        "Ionization energies": data.ionization_energies.join(', '),
